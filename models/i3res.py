...
class I3ResNet(torch.nn.Module):
    def __init__(self, resnet2d):
        ...
        self.layer4 = inflate_reslayer_l4(resnet2d.layer4)
...
def inflate_reslayer_l4(reslayer2d):
    reslayers3d = []
    count = 0
    for layer2d in reslayer2d:
        if count == 0:  # the first bottleneck with downsample
            layer3d = Bottleneck3d_l4(layer2d)
        else:
            layer3d = Bottleneck3d_l4_sub(layer2d)
        reslayers3d.append(layer3d)
        count += 1
    return torch.nn.Sequential(*reslayers3d)
...
class Bottleneck3d_l4(torch.nn.Module):
    def __init__(self, bottleneck2d):
        super(Bottleneck3d_l4, self).__init__()
        ...
        if bottleneck2d.downsample is not None:
            self.downsample = inflate_downsample_l4(
                bottleneck2d.downsample, time_stride=spatial_stride)
        else:
            self.downsample = None
...
class Bottleneck3d_l4_sub(torch.nn.Module):
    def __init__(self, bottleneck2d):
        super(Bottleneck3d_l4_sub, self).__init__()
...
def inflate_downsample_l4(downsample2d, time_stride=1):
    downsample3d = torch.nn.Sequential(
        inflate.inflate_conv(
            downsample2d[0], time_dim=1, time_stride=3, center=True),
        inflate.inflate_batch_norm(downsample2d[1]))
    return downsample3d
